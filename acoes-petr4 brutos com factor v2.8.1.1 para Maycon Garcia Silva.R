...
#treino   = 3118                #60% Melhores encaixes de distribuições no EasyFit (somente  treino   =60%):Johnson SB  γ=0,37905 δ=0,67725 λ=29,695 ξ=0,24026; Gen. Pareto k=-0,57119 σ=17,663 μ=0,91918; Power Function α=0,5191 a=2,4775 b=34,548
#valida   = 1040                #20% Melhores encaixes de distribuições no EasyFit (treino + validação=80%):Gen. Pareto k=-0,6386 σ=17,038 μ=1,6145; Johnson SB γ=0,5925 δ=0,99264 λ=34,873 ξ=-1,2082; Gen. Gamma (4P)	k=4,7245 α=0,16963 β=22,297 γ=2,4775
#20% Melhores encaixes de distribuições no EasyFit (somente  validação=20%):Gen. Pareto k=-1,249 σ=15,592 μ=4,6329; Johnson SB γ=0,23288 δ=0,78779 λ=15,347 ξ=3,0335; Error k=5,8807 σ=3,7225 μ=11,566
#teste    = 1040                #20%

treino   = 2600                #50% Melhores encaixes de distribuições no EasyFit (somente  treino   =50%):Power Function α=0,50339 a=2,4759 b=34,548; Dagum (4P) k=0,30596 α=2,0664 β=17,968 γ=2,4775; Johnson SB	γ=0,56742 δ=0,49965 λ=27,881 ξ=1,9165
valida   = 1299                #25% Melhores encaixes de distribuições no EasyFit (treino + validação=75%):Gen. Pareto    k=-0,71341 σ=18,607 μ=1,453; Error k=3,3537 σ=7,0701 μ=12,313; Johnson SB	γ=0,48101 δ=1,0128 λ=35,307 ξ=-1,9031
                               #25% Melhores encaixes de distribuições no EasyFit (somente  validação=25%):Gumbel Min     σ=2,5655 μ=15,991; Weibull (3P) α=16,43 β=45,26 γ=-29,296; Burr (4P) k=103,48 α=16,224 β=59,227 γ=-28,546
teste    = 1299                #25%
...
monitora <- function(obj){
  ...
  if (old_obj < obj@summary[[obj@iter]]){
     ...
     #write.table(linha_fitness,file = "Dados PETR4 melhores_fitness ESN_mae_otim40X60 com_factor 10000_1.csv", append = T, col.names = F, sep = "\t", eol = "\n")
     ...
     write.table(linha_bestSol,file = "Dados PETR4 bestSol melhores_fitness mae_otim40x60 com_factor 10000_1.csv", append = T, row.names = F, col.names = F, sep = "\t", eol = "\n")     
...
f <- function(cromossoma=c()){
   ...
   #Win_GA = matrix(rpearsonV(tam_reservoir_GA*(1+inSize),shape = , location = , scale = ),tam_reservoir_GA)
   #distr_Win_GA = "Pearson V shape= ,location=0,scale= "
   #Win_GA = matrix(rcauchy(tam_reservoir_GA*(1+inSize),location= ,scale= ),tam_reservoir_GA)
   #distr_Win_GA = "Cauchy location= ,scale= "
   ...
   #W_GA = matrix(rf(tam_reservoir_GA*tam_reservoir_GA,df=df_t),tam_reservoir_GA)
   #W_GA = matrix(rcauchy(tam_reservoir_GA*tam_reservoir_GA, location=W_C_loc, scale = W_C_sca),tam_reservoir_GA)
...
#Fitness = erro de treinamento * 0.4 e erro de validação * 0.6
   ...
   if (old_otim < otimiza){
      ...
      write.table(linha_entrada,     file = "Dados PETR4 Win ESN_mae_otim40X60 com_factor 10000_1.csv", append = T, col.names = F, sep = "\t", eol = "\n")
      linha_reservatorio = cbind(contar,W_GA_t, distr_W_GA)
      write.table(linha_reservatorio,file = "Dados PETR4 W reservatório ESN_mae_otim40X60 com_factor 10000_1.csv", append = T, col.names = F, sep = "\t", eol = "\n")
      linha_saida        = cbind(contar,Wout_GA_t)
      write.table(linha_saida,       file = "Dados PETR4 Wout ESN_mae_otim40X60 com_factor 10000_1.csv", append = T, col.names = F, sep = "\t", eol = "\n")
      old_otim         <<- otimiza
     }
   ...
   write.table(linha_ESN,         file = "Dados PETR4 ESN_mae_otim40X60 com_factor 10000_1.csv", append = T, col.names = F, sep = "\t", eol = "\n")
...
write.table(linha_ESN,file     = "Dados PETR4 ESN_mae_otim40x60 com_factor 10000_1.csv", append = F, row.names = F, col.names = F, sep = "\t", eol = "\n")
linha_bestSol      = cbind("Época","a","sr","iL","tr","reg","Fitness")
write.table(linha_bestSol,file = "Dados PETR4 bestSol melhores_fitness mae_otim40x60 com_factor 10000_1.csv", append = F, row.names = F, col.names = F, sep = "\t", eol = "\n")     
linha_entrada      = cbind("Época","Win","Distribuição")
write.table(linha_entrada,file = "Dados PETR4 Win ESN_mae_otim40X60 com_factor 10000_1.csv", append = F, col.names = F, sep = "\t", eol = "\n")
linha_reservatorio = cbind("Época","W","Distribuição")
write.table(linha_reservatorio,file = "Dados PETR4 W reservatório ESN_mae_otim40X60 com_factor 10000_1.csv", append = F, col.names = F, sep = "\t", eol = "\n")
linha_saida        = cbind("Época","Wout")
write.table(linha_saida,file = "Dados PETR4 Wout ESN_mae_otim40X60 com_factor 10000_1.csv", append = F, col.names = F, sep = "\t", eol = "\n")
...
itera     = 10000L      #Número de épocas máximo
...
#Trocar pela maior época obtida no gráfico
itera   = 9220

#Finalização dos arquivos que gravaram a ESN
linha_ESN          = cbind("Parâmetro","Contar","a","sr","initLen","tam_reservoir","reg","MAE_treino40%","MAE_valida60%","Otimiza","old_Otimiza","RMSE_treino","RMSE_valida","RMSE_t0.4+RMSE_v0.6")
write.table(linha_ESN,file     = "Dados PETR4 ESN_mae_otim40x60 com_factor 10000_1.csv", append = T, row.names = F, col.names = F, sep = "\t", eol = "\n")
...
write.table(linha_resumo,file  = "Dados PETR4 resumo fitness ESN_mae_otim40X60 com_factor 10000_1.csv", append = F, col.names = F, sep = "\t", eol = "\n")
linha_resumo       = cbind(sumario)
write.table(linha_resumo,file  = "Dados PETR4 resumo fitness ESN_mae_otim40X60 com_factor 10000_1.csv", append = T, col.names = F, sep = "\t", eol = "\n")
linha_resumo       = cbind("Melhor_Fitness","Fitness_médio","Fitness_mínimo")
write.table(linha_resumo,file  = "Dados PETR4 resumo fitness ESN_mae_otim40X60 com_factor 10000_1.csv", append = T, col.names = F, sep = "\t", eol = "\n")

linha_bestSol      = cbind("Época","a","sr","iL","tr","reg")
write.table(linha_bestSol,file = "Dados PETR4 bestSol mae_otim40x60 com_factor 10000_1.csv", append = F, row.names = F, col.names = F, sep = "\t", eol = "\n")
for (i in 1:itera) {
    ...
    write.table(linha_bestSol,file = "Dados PETR4 bestSol mae_otim40x60 com_factor 10000_1.csv", append = T, row.names = F, col.names = F, sep = "\t", eol = "\n")
}
linha_bestSol = cbind("Época","a","sr","iL","tr","reg")
write.table(linha_bestSol,file = "Dados PETR4 bestSol mae_otim40x60 com_factor 10000_1.csv", append = T, row.names = F, col.names = F, sep = "\t", eol = "\n")
