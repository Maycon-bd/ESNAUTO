...
# =============================================================================
# FUNÇÃO CENTRAL DE TREINAMENTO GRU (reutilizável)
# =============================================================================

treinar_modelo_gru <- function(dados, units = 50, timesteps = 10, dropout = 0.2,
  ...
  if (is.function(set_progress)) set_progress(0.1, "Preparando dados para GRU...")
  ...
  # Construir modelo
  if (is.function(set_progress)) set_progress(0.2, "Construindo arquitetura GRU...")
  
  model <- keras_model_sequential()
  if (segunda_camada) {
    model %>%
      layer_gru(units = units, input_shape = c(timesteps, 1), return_sequences = TRUE) %>%
      layer_dropout(rate = dropout) %>%
      layer_gru(units = units2, return_sequences = FALSE) %>%
      layer_dropout(rate = dropout) %>%
      layer_dense(units = 1)
  } else {
    model %>%
      layer_gru(units = units, input_shape = c(timesteps, 1), return_sequences = FALSE) %>%
      layer_dropout(rate = dropout) %>%
      layer_dense(units = 1)
  }
  ...
  if (is.function(set_progress)) {
    set_progress(0.05, "Iniciando treinamento da rede GRU...")
    ...
    fluidRow(
      column(4,
        div(class = "well model-card-gru",
          ...
          h4(style = "margin-top:0; color: var(--gru-color); font-weight: 800;", "📉 GRU Architecture"),
          hr(),
          ...
          hr(),
          actionButton(ns("btn_rodar"), "🚀 Treinar Modelo GRU", 
...
gru_server <- function(id, dados_reativo, resultados_externos = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$btn_rodar, {
      ...
      withProgress(message = "Treinando GRU...", value = 0, {
        ...
        salvar_resultado_dl("GRU", res$validacao$metricas, res$teste$metricas, res$tempo, epochs = input$epochs, timesteps = input$timesteps, units = input$units)
      })
    ...
    output$grafico_validacao <- renderPlot({
      ...
      plot(real, type = 'l', col = '#0f172a', lwd = 2,
           ...
           main = "GRU — Previsão na Fase de Validação", axes = FALSE, col.main = "#0f172a")
      axis(1, col = "#cbd5e1", col.axis = "#475569")
      axis(2, col = "#cbd5e1", col.axis = "#475569")
      grid(col = "#e2e8f0", lty = "dotted")
      
      lines(prev, col = '#7c3aed', lwd = 1.8)
      legend('topright', legend = c('Série Real', 'GRU Previsto'),
             col = c('#0f172a', '#7c3aed'), lty = 1, lwd = c(2, 1.8), bty = 'n', text.col = '#0f172a')
    })
    ...
    output$grafico_teste <- renderPlot({
      ...
      plot(real, type = 'l', col = '#0f172a', lwd = 2,
           ylab = "Preço PETR4 (R$)", xlab = "Dias",
           main = "GRU — Previsão na Fase de Teste", axes = FALSE, col.main = "#0f172a")
      axis(1, col = "#cbd5e1", col.axis = "#475569")
      axis(2, col = "#cbd5e1", col.axis = "#475569")
      grid(col = "#e2e8f0", lty = "dotted")
